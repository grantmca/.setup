create_layer() {
  ...
  if [ ! -d "$src_root/$layer" ]; then
    echo "[ERROR][create_layer ] Layer '$layer' not found in '$src_root'"
    exit 1
  fi

  if [ "${DRY_RUN:-0}" -eq 1 ]; then
    echo "[DRY-RUN][create_layer ] Would stow layer '$layer' from '$src_root' to '$HOME'"
    return 0
  fi
  ...
  if echo "$stow_output" | grep -qE '^(LINK|SYMLINK):'; then
    ...
    echo "[INFO][create_layer ] Layer '$layer' added successfully."
  else
    echo "[INFO][create_layer ] Layer '$layer' already exists and is up to date."
  fi

}

# Subcommand: create-layer <layer-name>
create_layer_cmd() {
  local layer="$1"
  if [ -z "$layer" ]; then
